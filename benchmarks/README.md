...
> [`results/CONCLUSION.txt`](results/CONCLUSION.txt): **90 ops improved, 24
> unchanged, 0 regressed**; `dg check` at 5,000 vertices went 132.5 s → 1.6 s
> and its exponent 3.03 → 1.41. The account of what changed and why is
...
| `results/final.txt` | The acceptance run after fix D |
| `results/CONCLUSION.txt` | **The final numbers, and where the audit stopped** |
| `results/two-graph.txt` | Both stores large and dense together |
| `results/layout.txt` | Routing long edges around the boxes they crossed |
