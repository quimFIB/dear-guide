#!/usr/bin/env bash
# Scene 6 — bringing the parallel work back together.
#
...
scene "Scene 6a — what git can do with two agents' work"
...
conflict. **This is what isolation cost**: scenes 2 and 3 cannot happen in
separate clones, and this can. The race did not go away, it moved.
...
Two additions is the case a union genuinely handles. The next scene is the case
it does not."

scene "Scene 6b — the same question, answered twice"
own_clones
beat_the_day_so_far "$A_DIR"
pull "$B_DIR"; pull "$C_DIR"
say "Both clones now hold the day's work — D01 re-answered at 40 MB, D02
settled, D03 reopened when its falsifier fired.
