...
The model, the repo, every command, and every rule `dg check` enforces. For the
story behind any of it, read [how it works](how-it-works.md) first; this page
assumes you want the details in one place.
...
Every command, with the flag that expands its output where it has one.
`dg <command> --help` is complete; this is the index.
...
- [How it works, and why](how-it-works.md) — the same model told as one
  project's story, from first question to first reversal.
- [The CLI quick start](quickstart-cli.md) — the commands in the order you meet
  them.
- [The design behind `dg find`](query-framework.md) — why the query language is
  shaped the way it is.
- [Composing in an editor](emacs.md) — the org buffer, its keys, and what
  happens to org markup on the way into a view.
- [Still open](open-questions.md) — the questions this design has not settled.
