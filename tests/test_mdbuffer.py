...
def test_seeded_prose_round_trips_through_both_escapes(g, store):
    """`_escape` comma-escapes a `*` line for org; the markdown buffer undoes
    that and escapes `#` instead. Both are undone on the way back."""
    seed = {"answer": "*a bullet*\n,*already commaed\nplain",
            "source": "s", "falsifier": "f"}
    md = mdbuffer.render(editor.render_close(g, "D05", seed))
    assert "\n*a bullet*\n,*already commaed\nplain\n" in md
    (op,) = editor.parse(md, g=g, dialect="markdown")
    assert op["answer"] == seed["answer"]
